 ...
 *
 *
 *	Changes by Juergen Mueller, juergen@e-basteln.de:
 * 
 * jm 24.4.2020	Add SYNC output, high while reading instruction data
 * jm 15.6.2020	SYNC output high one cycle earlier (while instruction address is on the bus)
 * jm 15.6.2020	Add SO input, low > high transition sets overflow bit
 * jm 10.7.2020	Add outputs indicating RTS, RTI, JSR 
 * jm 27.7.2020	Set outputs indicating RTS, RTI, JSR only when RDY
 * jm  7.8.2020	Combine RTS/RTI, JRS/BRK/interrupt signals
 */
...
module cpu_65c02( clk, reset, AB, DI, DO, WE, IRQ, NMI, RDY, SYNC, SO, isRTS, isJSR );
...
output SYNC;				// early SYNC signal. jm 24.4.2020 
input SO;					// add SO input. jm 15.6.2020
output isRTS;				// processing RTS/RTI instruction. jm 10.7.2020
output isJSR;				// processing JSR/BRK instruction, IRQ, NMI. jm 10.7.2020
...
/*
 * Drive SYNC output - jm 24.4.2020, 15.6.2020
 */
reg SYNC;
always @*
	if (
		(state == FETCH) || (state == REG) || (state == PUSH1) || (state == PULL2) 
		|| (state == RTI4) || (state == BRA2)  || (state == JMP1)
		|| ((state == BRA0) && cond_true)
		|| ((state == BRA1) && (CO == backwards))
		) 
		SYNC = 1;
	 else 
		SYNC = 0;
...
/*
 * Drive isRTS, isJSR outputs - jm 10.7.2020, 7.8.2020
 */

reg isRTS;
always @(posedge clk )
	if (RDY && ((state == RTS0) || (state == RTI0))) isRTS = 1;
	else isRTS = 0;

reg isJSR;
always @(posedge clk )
	if (RDY && ((state == JSR0) || (state == BRK0))) isJSR = 1;
	else isJSR = 0;
...
// SO input handling jm 15.6.2020

reg SO_1 = 0;          // delayed SO signal

always @(posedge clk)
    SO_1 <= SO;

assign SO_edge = ( SO & ~SO_1 );
